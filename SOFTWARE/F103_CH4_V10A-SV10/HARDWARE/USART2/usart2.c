...
* 函数名: 串口发送函数
* 功  能: Uart2Sends函数
* 输  入: char* data_at 字符串头地址
* 输  出: None
* 返回值: None
*******************************************************************************/
void Uart2Sends(u8* data_at)
{
	u8 cnt=0;
 	while(*(data_at+cnt))
	{
		USART_SendData(USART2,*(data_at+cnt));  //从串口1发送数据
        while(USART_GetFlagStatus(USART2, USART_FLAG_TC) == RESET){}  //等待一个发送完一个数据
		cnt++;
	}
    delay_us(100);
	cnt=0;
